<script lang="ts">
    ...
    onMount(async () => {
        user_operators = await getUsersByRole('maintenance_operator');
        ...
        <label>
            <select bind:value={maintenance_request_id} required>
                ...
                {#each maintenance_requests as maintenance_request}
                    <option value={maintenance_request.id}>{maintenance_request.description}</option>
                {/each}
        </label>
        <br />
        ...
        <label>
            <select bind:value={user_operator_id} required>
                ...
                {#each user_operators as user_operator}
                    <option value={user_operator}>{user_operator}</option>
                {/each}
        </label>
        <br />
